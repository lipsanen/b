...
bdemo::error_code parser_t::parse(ireader_t* reader, demo_t* output)
{
    parser_t parser;
    ...
    auto result = bdemo::error_code::success;

    while(result == bdemo::error_code::success) {
        result = parser.parse_anymessage();
    }

    if(result == bdemo::error_code::eof) {
        result = bdemo::error_code::success;
    }
...
void parser_t::parse_header()
{
    ...
    m_pOutput->m_version = bdemo::get_demo_version(header);
}

bdemo::error_code parser_t::parse_anymessage()
{
    ...
    switch(type) {
        ...
        case 9:
            return parse_stringtables();
        default:
            return bdemo::error_code::bad_message;
    }
}

static void parse_preamble(parser_t* parser, bdemo::message_preamble* preamble) {
...
template<typename T> bdemo::error_code _parse_data(parser_t* parser, T& message)
{
    ...
    if(message.size_bytes > 0) {
        ...
        if(message.data != nullptr) {
            ...
            return bdemo::error_code::invalid_data;
        }
    ...
    return bdemo::error_code::success;
}

bdemo::error_code bdemo::parser_t::parse_consolecmd() {
    bdemo::consolecmd_t message;
...
bdemo::error_code bdemo::parser_t::parse_datatables() {
    bdemo::datatables_t message;
    parse_preamble(this, &message.preamble);
    auto result = _parse_data(this, message);
...
bdemo::error_code bdemo::parser_t::parse_packet(messagetype_t type) {
    bdemo::packet_t packet;
...
bdemo::error_code bdemo::parser_t::parse_stop() {
    stop_t message;
    m_pOutput->m_vec_messages.push_back(std::move(message));
    return bdemo::error_code::eof;
}

bdemo::error_code bdemo::parser_t::parse_synctick() {
    synctick_t message;
    parse_preamble(this, &message.preamble);
    m_pOutput->m_vec_messages.push_back(std::move(message));
    return bdemo::error_code::success;
...
bdemo::error_code bdemo::parser_t::parse_usercmd() {
    usercmd_t message;
    parse_preamble(this, &message.preamble);
...
bdemo::error_code bdemo::parser_t::parse_stringtables() {
...
bdemo::error_code bdemo::parser_t::parse_customdata() {
...
bdemo::error_code demo_t::parse_from(const char* filepath, demo_t* output)
{
    filereader_t reader;
    if(!reader.try_open(filepath)) {
        return bdemo::error_code::no_such_file_or_directory;
    }
...
demomessage_t bdemo::clone_message(const demomessage_t* msg)
{
    return std::visit([](auto& arg) -> demomessage_t { return arg.clone(); }, *msg);
}
    
void bdemo::free_message(demomessage_t* msg)
{
