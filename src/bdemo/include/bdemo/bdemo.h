...
namespace bdemo
{
    ...
    #define BDEMO_MACRO_ALL_MESSAGES(macro) \
        macro(net_nop)  \
    ...
    #define BDEMO_DECLARE_ENUM(x) x,

    enum class net_message_t { BDEMO_MACRO_ALL_MESSAGES(BDEMO_DECLARE_ENUM) svc_invalid };
    ...
    struct packet_t {
        ...
        void destroy();
        message_preamble preamble;
        cmdinfo cmdinfo[4]; // Some games have multiple cmdinfos, split screen
    ...
    struct demo_t
    {
        ...
        demo_t() = default;
        ~demo_t();
        static bdemo::error_code parse_from(const char* filepath, demo_t* output);
    };

    // Version utils
    version_t get_demo_version(bdemo::header_t *header);
    void version_update_build_info(bdemo::version_t *version);
